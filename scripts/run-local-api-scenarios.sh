...
# Read from the environment, never embedded. On a fresh database this is the username ('admin');
# after the forced first-sign-in change it is whatever the operator chose.
: "${MCP_ADMIN_PASSWORD:?set MCP_ADMIN_PASSWORD to the admin password before running this}"
request valid_login 200 POST /api/auth/login "{\"username\":\"admin\",\"password\":\"${MCP_ADMIN_PASSWORD}\"}"
