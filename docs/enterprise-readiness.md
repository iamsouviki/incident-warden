...
| C10 | ~~`RagService.searchWeb()` scraped `html.duckduckgo.com` with a spoofed Chrome user agent.~~ **Fixed.** Web search is removed from services, controller, config service, config key, frontend and docs; `AiConfigService` no longer has a reader or writer for `web_search_enabled`, so it cannot be re-enabled by flag. | `RagService.java`, `AiConfigService.java`, `AiConfigController.java` | Resolved |
