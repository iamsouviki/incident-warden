...
import com.company.warden.repository.HitlRequestRepository;
import com.company.warden.repository.IncidentRepository;
import com.company.warden.repository.RemediationPlanRepository;
...
public class HitlWorkflowService {
    ...
    private final com.company.warden.repository.UserRepository userRepository;
    private final com.company.warden.repository.SystemConfigRepository config;
    ...
    public HitlWorkflowService(IncidentRepository incidents, RemediationPlanRepository plans, HitlRequestRepository requests,
                               ...
                               com.company.warden.repository.UserRepository userRepository,
                               com.company.warden.repository.SystemConfigRepository config) {
    ...
    public boolean separationOfDuties() {
        return config.findById(SEPARATION_KEY).map(com.company.warden.model.SystemConfig::getConfigValue)
                .map(Boolean::parseBoolean).orElse(separationOfDutiesRequired);
    }

    public void setSeparationOfDuties(boolean value) {
        config.save(new com.company.warden.model.SystemConfig(SEPARATION_KEY, Boolean.toString(value)));
    }
