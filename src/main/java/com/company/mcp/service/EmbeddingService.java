 ...
 *
 * <p>Uses a deterministic mock vector based on text hash. For production RAG
 * quality, configure an external embedding API via the embeddings endpoint
 * and override {@link #generateEmbedding(String)}.</p>
 *
 * <p>The mock embeddings are 1536-dimensional and deterministic — identical
 * text always produces the same vector, enabling consistent similarity scoring.
 */
@Slf4j
@Service
public class EmbeddingService {
    ...
    public String generateEmbedding(String text) {
        ...
        log.debug("Generating mock embedding for text ({} chars)", text.length());
        return generateMockEmbedding(text);
    }
    ...
    public List<String> generateEmbeddings(List<String> texts) {
        if (texts == null || texts.isEmpty()) return List.of();
        List<String> result = new ArrayList<>(texts.size());
        for (String t : texts) result.add(generateMockEmbedding(t));
        return result;
    }
